...
describe("token", function () {
  ...
  async function deploy() {
    ...
    const Token_deploy = await tokenDeploy.deploy("ONESTEP","OST");
...
describe("treenft", function () {
  ...
  async function deploy() {
    ...
    const treenft_deploy = await treeNFTDeploy.deploy();
...
describe("certnft", function () {
  ...
  async function deploy() {
    ...
    const certnft_deploy = await certNFTDeploy.deploy();
...
describe("operator", function () {
  ...
  async function deploy() {
    ...
    const operator_deploy = await operatorDeploy.deploy("0x3A6BD6182951AFdB2f2087731711160c7310aD00","0x6480521d388B208Bc1348dE6Fba3216Fd2E8F7Ba","0x83B08261CeB002EE7664955a6B650B254722E21b");
  ...
  /*describe("UpdateValues", function () {
    const connex = new Connex('https://sync-testnet.vechain.org');
    const contractABI = 
    [
      {
        "inputs": [],
    ...
    const contractData = connex.thor.account("0xdFE3ce6336030C55C57A9154809b194f0770ad66").asContract(contractABI);

      const tx = connex.vendor.sign('tx');
      const clause = connex.thor.account(operator.address).method('rewardTokens', account, amount).asClause();
      tx
          .from(wallet)
          .to(clause)
          .gas(50000) // Specify the gas limit
          .gasPriceCoef(0) // Specify the gas price coefficient
